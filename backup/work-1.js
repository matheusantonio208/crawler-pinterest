/*'https://br.pinterest.com/matheusantoni0/profissional-artesanato/to-download/'
https://br.pinterest.com/matheusantoni0/corpo/*/
const fs = require('fs');
const request = require('request');
const puppeteer = require('puppeteer');

const email = 'matheus.antonio208@gmail.com';
const password = 'Geniodanet1!';
const linkBoardToDownload = 'https://br.pinterest.com/matheusantoni0/profissional-artesanato/to-download/';

async function startWithLogin(page) {
  await page.setViewport({width: 1080, height: 10000000});
  await page.setDefaultNavigationTimeout(0);
  await page.goto('https://br.pinterest.com/login/', { waitUntil: 'networkidle0' });
  await page.type('#email', email);
  await page.type('#password', password);

  await Promise.all([
    page.click('.SignupButton'),
    page.waitForNavigation()
  ])
}

async function getImages(page) {
  await page.evaluate(() => {
    const nodeList = document.querySelectorAll('[src^="https://i.pinimg.com/"]');
    const imageArray = [...nodeList];
    const img = imageArray.map( ({src}) => ({
      src
    }))

    return img;
  });
}

function downloadImage(imageList){
  const download = (url, path, callback) => {
    request.head(url, (err, res, body) => {
      request(url)
        .pipe(fs.createWriteStream(path))
        .on('close', callback)
    })
  }

  imageList.forEach(img => {
    const url = img.src;
    const path = './images/image.png'

    download(url, path, () => {
      console.log('`${url}` ✅ Done!')
    })
  });
}
  
(async () => {
  const browser = await puppeteer.launch({headless: false});
  const page = await browser.newPage();
  
  await startWithLogin(page);
  
  await Promise.all([
    page.goto(linkBoardToDownload,{timeout: 0, waitUntil: 'domcontentloaded'}),
    page.waitForNavigation({waitUntil: 'networkidle2'})
  ])

  const imageList = await getImages(page);
  console.log(imageList);

  downloadImage(imageList);
})();

